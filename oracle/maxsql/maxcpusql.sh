...
if [ $# -lt 1 ]; then
  echo "Usage: sh maxcpusql.sh system/system_pwd"
  exit
fi

#result file
sqlplan_file=${CURR_DIR}/tmp/sqlplan_cpu.tmp.$(date "+%Y%m%d%H%M%S")

#获取pid列表
pids=$(top -b -n 5 | awk -v pct=${pct} -F' ' '{if(match($1, /[0-9]+/) && $9>pct){print $1}}' | sort | uniq | sed '/^$/d' | tr \\n ,)
pids=${pids%,}
