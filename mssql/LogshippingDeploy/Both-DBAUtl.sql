...
IF NOT EXISTS(SELECT *
              FROM sys.objects
              WHERE object_id = OBJECT_ID(N'[dbo].[dba_FileExist]')
                AND type in (N'P', N'PC'))
    BEGIN
        EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[dba_FileExist] AS'
    END
GO

ALTER PROCEDURE [dbo].[dba_FileExist]
...
IF NOT EXISTS(SELECT *
              FROM sys.objects
              WHERE object_id = OBJECT_ID(N'[dbo].[dba_GetDefaultBackupDir]')
                AND type in (N'P', N'PC'))
    BEGIN
        EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[dba_GetDefaultBackupDir] AS'
    END
GO

ALTER PROCEDURE [dbo].[dba_GetDefaultBackupDir]
    @DefaultBackupDir varchar(128) output
    AS
    BEGIN
...
IF NOT EXISTS(SELECT *
              FROM sys.objects
              WHERE object_id = OBJECT_ID(N'[dbo].[dba_DeleteBackupFile]')
                AND type in (N'P', N'PC'))
    BEGIN
        EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[dba_DeleteBackupFile] AS'
    END
GO

ALTER PROCEDURE [dbo].[dba_DeleteBackupFile]
    @LogshippingDBInitDir varchar(1024),
